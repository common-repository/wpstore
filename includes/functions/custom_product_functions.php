...
// FINAL FUNCTION custom_get_select_stock_form
 ...
 function custom_get_select_stock_formB($postID,$showButtom=true){
     ...
     include('layout/box-comprarB.php');
 ...
 function custom_get_select_stock_formChange($postID,$showButtom=true){
     ...
     if(intval($ilimitado)==1 || $ilimitado=="" ){
      ...
      include('layout/box-comprarChange.php');
 ...
 function custom_get_qtd_stock($postID,$cor,$tamanho,$controle=false){
     ...
     if($variacao=='unico'){
         ...
         $qtdReservaUsuario = intval(custom_get_stock_reservaUsuario($postID,''));
      ...
      //echo  $qtdStock."yyyyyyy" ;
	 
     $qtdProdIilimitado = get_post_meta($postID, 'is_check_outofstock', true);
     //echo  $qtdProdIilimitado."ZZZZ" ;
      if( $qtdProdIilimitado== '1' ||  $qtdProdIilimitado == ''){
            if($controle==false){ 
			  $qtdStock =  100000000; 
		   }; 
      }
...
$sql = "SELECT * FROM `$tabela` WHERE `id_produto` LIKE '$postID' AND `variacao` = '$tamanho-$cor'"; 
	...
	if($cor=="" && $tamanho !=""){
        ...
        $sql = "SELECT * FROM `$tabela` WHERE `id_produto` LIKE '$postID' AND `variacao` REGEXP '$cor$' ";    
          ...
          $sql = "SELECT * FROM `$tabela` WHERE `id_produto` LIKE '$postID' AND `variacao` = '$cor'";    
	}
   */
 
    $sql =  'SELECT qtdProd FROM `'.$tabela.'` pro INNER JOIN `'.$tabelaOrder.'` ord ON pro.id_pedido = ord.id_pedido WHERE pro.id_produto = '.$postID.' AND ord.status_pagto != "CANCELADO" ';
	

 	 if($cor=="" && $tamanho !=""){
 
		   $sql =  'SELECT qtdProd FROM `'.$tabela.'` pro INNER JOIN `'.$tabelaOrder.'` ord ON pro.id_pedido = ord.id_pedido WHERE pro.id_produto='.$postID.'  AND pro.variacao REGEXP "'.$tamanho.'"  AND ord.status_pagto != "CANCELADO" ';
		  
     }elseif($tamanho=="" && $cor !=""){
 
	   $sql =  'SELECT qtdProd FROM `'.$tabela.'` pro INNER JOIN `'.$tabelaOrder.'` ord ON pro.id_pedido = ord.id_pedido WHERE pro.id_produto='.$postID.'  AND pro.variacao REGEXP "'.$cor.'$"  AND ord.status_pagto != "CANCELADO" ';
	     
	  /// echo "--$cor --- $postID ";
	   
   }elseif($tamanho!="" && $cor !=""){

   $sql =  'SELECT qtdProd FROM `'.$tabela.'` pro INNER JOIN `'.$tabelaOrder.'` ord ON pro.id_pedido = ord.id_pedido WHERE pro.id_produto='.$postID.'  AND pro.variacao = "'.$tamanho.'-'.$cor.'"  AND ord.status_pagto != "CANCELADO" ';
	   ...
	   if($cor==""){
		   ...
		   $sql =  'SELECT qtdProd FROM `'.$tabela.'` pro INNER JOIN `'.$tabelaOrder.'` ord ON pro.id_pedido = ord.id_pedido WHERE pro.id_produto='.$postID.'  AND pro.variacao = "'.$cor.'"  AND ord.status_pagto != "CANCELADO" ';
...
function custom_get_stock_reservaUsuario($postID,$variacao){
    
    if( $variacao=="" || $variacao=="-" ){ $variacao=''; };
     ...
     if(count($arrayCarrinho)>0){
     	  foreach($arrayCarrinho as $item){
          	if( $item['prodString'] == trim($postID.$variacao) ){
...
//GERAR PEDIDO ----------------------------------------------------------------------------------
 function gerarPedido($idUsuario,$valor,$frete,$tipoPagamento,$statusPagamento,$comentarioAdmin,$extra=''){
     
		  
	      //echo "DADOS A GRAVAR : $idUsuario,$valor,$frete,$tipoPagamento,$statusPagamento,$comentarioAdmin";
        ...
        $num = $wpdb->get_var(   "SELECT COUNT(*) FROM  `$tabela` WHERE `id_usuario`='$idUsuario' "  );
				
				//echo "TOTA ORDERS USUARIO: $num**";
        ...
        $id = intval( $wpdb->get_var(  "SELECT `id` FROM `$tabela` WHERE `id_pedido`='$idPedido' " ) );
      ...
      if($id ==0){
       ...
       $sql = "INSERT INTO `$tabela` (`id` ,`id_pedido` ,`id_usuario` ,`valor_total` ,`frete` ,`tipo_pagto` ,`status_pagto` ,`comentario_cliente`,`comentario_admin`,`comentario_pagt`)
              VALUES ( NULL , '$idPedido', '$idUsuario', '$valor', '$frete', '$tipoPagamento', '$statusPagamento', '' , '$comentarioAdmin' , '$extra');
       ";
    
       $resultQuery = $wpdb->query($sql);
			 
       $lastid  = $wpdb->get_var(  "SELECT `id_pedido` FROM `$tabela` ORDER BY `id` DESC LIMIT 0 , 1" );
...
//visualizar Pedidos --------------------------------
     ...
     function  custom_get_order_user($print=true,$orderNum="", $pagto="on" , $sendEmail=false){
...
function custom_get_total_products_in_order($idPedido){
 ...
 function gravarSolicitacaoContato($nomeAviso,$emailAviso,$telefoneAviso,$postIDP,$variacaoCorP,$variacaoTamanhoP){
           ...
           $sql = "INSERT INTO `$tabela` 
                   (`id` ,`nomeAviso` ,`emailAviso` ,`telefoneAviso` ,`postIDP` ,`variacaoCorP`,`variacaoTamanhoP`)
                   VALUES 
                   ( NULL ,'$nomeAviso','$emailAviso','$telefoneAviso','$postIDP','$variacaoCorP','$variacaoTamanhoP');
            ";
 ...
 function getCupomInfos($info){
   ...
   function addUseCupom($info){
    ...
    function verifyURL($url){
        ...
        if($ativarssl=="sim" || $ativarssl=="yes"){ 
	...
	function edit_qtd_stock($itemID,$variacao,$qtdProduto){
		
		if($variacao=='unico'){
			update_post_meta($itemID,'initstock',$qtdProduto);	
            $msg = "Atualizado com Sucesso!";
            return  $msg;
		}elseif($variacao=='tamanho' || $variacao=='cor'  || $variacao=='tamanhoCor' ){
			  global $wpdb; 
        	  $tabela = $wpdb->prefix."";
         	  $tabela .=  "wpstore_stock";
             $sql = "UPDATE `$tabela` SET  `qtdProduto` = '$qtdProduto' WHERE  `id` = '$itemID'; ";
		      $resultQuery = $wpdb->query($sql);
		      $msg = "Atualizado com Sucesso!";
              return  $msg;
	     };
    ...
    //FUNCAO ALTERA STATUS Pergunta--------------------------------------------------------

    function alteraPerguntaStatus($ID,$status="" ,$comentario ){
                  ...
                  $resultQuery = $wpdb->query("UPDATE `$tabela` SET `comentario_status` = '$status' ,  `comentario_cliente` = '$comentario'     WHERE `id` = '$ID' ");
    };

    //FINAL FUNCAO ALTERA STATUS pergunta ---------------------------------------------------
    ...
    //FUNCAO ALTERA COMENTARIo  Pergunta--------------------------------------------------------

    function alteraPerguntaComentario($ID, $comentario ){
                  ...
                  $resultQuery = $wpdb->query("UPDATE `$tabela` SET   `comentario_cliente` = '$comentario'  WHERE `id` = '$ID' ");
    };

    //FINAL FUNCAO ALTERA COMENTARIo pergunta ---------------------------------------------------
    ...
    //FUNCAO ALTERA STATUS avaliacao--------------------------------------------------------

       function alteraAvaliacaoStatus($ID,$status="" ,$comentario ){
                     ...
                     $resultQuery = $wpdb->query("UPDATE `$tabela` SET `comentarioStatus` = '$status' ,  `comentarioCliente` = '$comentario'     WHERE `id` = '$ID' ");
       };
       ...
       function alteraAvaliacaoComentario($ID, $comentario ){
                     ...
                     $resultQuery = $wpdb->query("UPDATE `$tabela` SET   `comentarioCliente` = '$comentario'  WHERE `id` = '$ID' ");
       };
    ...
    function enviaEmailStatus($orderComplete){
        ...
        // ENVIO DE EMAIL ----------------------------------------------------------
           ...
           $mensagemEmail = "
              ...
              $statusOrder = get_status_order($orderComplete);
			  
		      if($statusOrder=='ENTREGUE'){
				  
 			 	$mensagemEmail.= "<p><strong>Avalie sua compra</strong>. 
					<a href='".get_bloginfo('url')."/avaliacao-de-produto/?order=$orderComplete'  title='Avaliar Compra'>Clique aqui</a> . 
				
				 Através de sua avaliação nós podemos oferecer produtos e  serviços cada vez melhores.<br/></p>";
             ...
             $mensagemEmailOrder = custom_get_order_user(false,$orderComplete,'off',true);
    ...
    function enviaEmailAdmin($orderComplete){
        ...
        // ENVIO DE EMAIL ----------------------------------------------------------
           ...
           $user_email =  get_option('emailAdminWPSHOP');
	...
	//GET STATUS OF ORDER ------------------------------
	  ...
	  function get_status_order($orderComplete){
	     global $wpdb;
         $tabela = $wpdb->prefix."";
         $tabela .=  "wpstore_orders";
         $fivesdrafts = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM  `$tabela` WHERE  `id_pedido`='$orderComplete' ORDER BY `id`  DESC " ,1,'') );
...
//GET STATUS OF ORDER ------------------------------
    ...
    function enviaEmailPergunta($usuarioNome,$usuarioEmail,$assuntoEmail,$conteudo){
    ...
    //FIND IDPAGE  ENTREGA ------------------------------
    function get_idPaginaEntrega(){
    $idPagina   =  intval(trim(get_option('idPaginaDetalhesEntregaWPSHOP')));
    
    if( $idPagina<= 0){
    global $wpdb;
    $shortcode = '[get_detalhes_entrega]';
    $query = "SELECT `ID`  FROM `$wpdb->posts` WHERE `post_content` LIKE '$shortcode' AND `post_type` = 'page' LIMIT 0,1";
    $idPagina = $wpdb->get_var($query);
   ...
   // echo "ID-$idPagina-DI";
    ...
    //FIND IDPAGE pedidos------------------------------
      ...
      //FIND IDPAGE pedidos------------------------------
        function get_idPaginaBoleto(){
        $idPagina   =  intval(trim(get_option('idPaginaBoletoWPSHOP')));

        if(  $idPagina <= 0 ){
        global $wpdb;
        $shortcode = '[custom_get_boleto]';
        $query = "SELECT `ID`  FROM `$wpdb->posts` WHERE `post_content` LIKE '$shortcode' AND `post_type` = 'page' LIMIT 0,1";
         $idPagina   = $wpdb->get_var($query);
        ...
        //FIND IDPAGE pedidos------------------------------
          function get_idPaginaObrigado(){
          $idPagina   =  intval(trim(get_option('idPaginaObrigadoWPSHOP')));

          if(  $idPagina <= 0 ){
          global $wpdb;
          $shortcode = '[custom_get_obrigado]';
          $query = "SELECT `ID`  FROM `$wpdb->posts` WHERE `post_content` LIKE '$shortcode' AND `post_type` = 'page' LIMIT 0,1";
           $idPagina   = $wpdb->get_var($query);
