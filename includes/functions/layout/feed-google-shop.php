...
$sql = 'post_type=produtos&showposts='.$numposts;

$catID = addslashes($_REQUEST['catID']);
//$cat = intval($catID);
  
if(intval($catID) >1){
$sql = "cat=$catID&post_type=produtos&showposts=$numposts";
};
 
$posts = query_posts($sql);
...
<channel>
  ...
  <managingEditor><?php echo get_option('emailAdminWPSHOP'); ?></managingEditor>    
   ...
   while ( have_posts() ) : the_post(); 
    ...
    if($specialprice !="" && intval($specialprice)>0){$preco = $specialprice; };  
...
<item>
   ...
   the_content_rss('', TRUE, '', 50);
	...
	<g:google_product_category>$googleCategories</g:google_product_category>
	...
	$rss .= "<!-- The following attributes are not required for this item, but supplying them is recommended if applicable -->
	<g:product_type>$categoria</g:product_type> ";
	 
	 
	 
	
	if(trim($gtim)=="" || trim($pmn)==""){
	 $rss .= "<g:identifier_exists>FALSE</g:identifier_exists>";
  };
