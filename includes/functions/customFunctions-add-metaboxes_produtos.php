...
$meta_boxes_produtos = array(
 ...
 "Estoque" => array(
  ...
  "title" => "Definir tipo de controle do estoque",
 ...
 */
 ...
 "description" => "É o texto  adicional  sobre garantias e suporte p/ do  produto. Para exibir o texto de garantia padrão   preencha com # .  "),
 
 
 "tabelaTamanho" => array(
 "name" => "tabelaTamanho",
  "title" => "Tabela de tamanho",
 "description" => "Url da imagem com a tabela de tamanho do produto. Para exibir a tabela padrão preencha com # . "),
 ...
 "entregaEspecial" => array(
 "name" => "entregaEspecialData",
  "title" => "Informar abaixo caso este produto tenha um prazo diferenciado para entrega.",
 "description" => "Coloque o tempo estimado para entrega . Ex: 15 dias úteis "),
 
 
 
 "freteGratisProduto" => array(
 "name" => "freteGratisProduto",
  "title" => "Frete Grátis para Estado**Cidade ou Estado**Estado .",
 "description" => "Coloque a dupla Estado**Cidade entre virgulas . Ex: RJ**Niterói ,RJ**São Gonçalo . Para escolher todas as cidades de um estado defina : UF**UF . ex: RJ**RJ"),
...
foreach( $meta_boxes_produtos as $meta_box ) {
  ...
  if($varUp != ""  ||  $meta_box[ 'name' ] =="banner" ||  $meta_box[ 'name' ]=="entregaEspecialData"  || $meta_box[ 'name' ] == 'freteGratisProduto' ){
...
<?php if($meta_box[ 'name' ]=="color"  ){
       ...
       <p     class='itemStock <?php echo $fivesdraftC->id; ?>'  rel='<?php echo $fivesdraftC->id; ?>' >Nome <?php echo $nome; ?>: <input type='text' name='<?php echo $nome; ?>Nome<?php echo $fivesdraftC->id; ?>' id="<?php echo $nome; ?>Nome<?php echo $fivesdraftC->id; ?>"  value="<?php echo $fivesdraftC->variacaoProduto; ?>" style="width:100px" />  
          ...
          QTD:<input type='text'  class="qtd" name='<?php echo $nome; ?>Qtd<?php echo $fivesdraftC->id; ?>' value="<?php echo $qtd; ?>" id="<?php echo $nome; ?>Qtd<?php echo $fivesdraftC->id; ?>" style="width:30px"/>
           ...
           <input type="button" id="<?php echo $nome; ?>Remove" name="<?php echo $nome; ?>Remove" style="width:110px" value="Remover"  class="removeITem"  rel='cor' rev="<?php echo $fivesdraftC->id; ?>" />
		    ...
		    //  echo " $post->ID , $fivesdraftC->variacaoProduto ";
 ...
 <?php 	if($controlarEstoque=='1'){
				
				echo "<p class='itemStock'  >Para poder adicionar estoque ilimitado para um produto, desative a opção 'Forçar Controle de estoque' nas opções gerais do wpstore. <a href='".get_bloginfo('url')."/wp-admin/admin.php?page=general'>Acesse aqui</a> . ";
        ...
        ILIMITADO : <input type="radio" class='radio' name="<?php echo $meta_box[ 'name' ]; ?>"  value="1"  <?php if( get_post_meta($post->ID, $nameValue, true)== '1' || get_post_meta($post->ID, $nameValue, true)== ''){  echo 'checked="checked"';  }; ?>  style="width:15px"/> 
           ...
           <?php }; ?> 
             ...
             <p  class='itemStock'  >    Controlar :<input type="radio" class='radio' name="<?php echo $meta_box[ 'name' ]; ?>"  value="0"  <?php if( get_post_meta($post->ID, $nameValue, true)== '0' || $controlarEstoque=='1'  ){  echo 'checked="checked"';  }; ?>  style="width:15px"/> 
