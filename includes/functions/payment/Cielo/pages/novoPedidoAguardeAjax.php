<?php  
 $ajaxC = addslashes(trim($_POST["ajaxC"])); 
if($ajaxC=='true'){
...
<?php
    ...
    $idPedido = $pedidoID;       //from saveOrderPayment.php
    
	$_SESSION['pedidoOrderID'] = $idPedido;
	...
	$Pedido->dadosPedidoDescricao =$idPedido;
     ...
     $totalCompraCielo = trim(getPriceFormat($valor));
	 ...
	 //echo "ZZZZZZ $totalCompraCielo ZZZZZZZ";
          
	 $Pedido->dadosPedidoValor = $totalCompraCielo; //from saveOrderPayment.php 
	
	//$ReturnURL = ReturnURL();
 
	
	$idPaginaPedido = get_idPaginaPedido();
	$urlPedido = trim(verifyURL(get_permalink($idPaginaPedido)));
	
	$ReturnURL  = $urlPedido."?orderCIELO=$pedidoID";
...
//	print_r($StrPedido);
	...
	echo $idPedido.'****'.$Pedido->urlAutenticacao.''; 
