<?php
      ...
      $fivesdrafts = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM  `$tabela` WHERE  `id_usuario`='$idUser' AND   `id_pedido`='$oid'  ORDER BY `id`  DESC " ,1,'') );
	...
	<tr> 
       ...
       <td class="hide-phone">
           <?php if($status_pagto=="APROVADO" || $status_pagto=="TRANSPORTADORA" || $status_pagto=="ENTREGUE"){  ?>  
       ...
       <?php if( $status_pagto=="PENDENTE"){ ?>
       <span class="detProd hide-phone" > <a href="<?php echo get_permalink($idPaginaPedido); ?>?order=<?php echo $idPedido; ?>#pagamento" target="_blank">Pagamento</a></span> 
       <?php }; ?>
